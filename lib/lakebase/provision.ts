...
// Incremented every time a genuinely new DB credential is minted.
...
async function createProject(): Promise<void> {
  ...
  if (resp.status === 409) {
    logger.info("Lakebase project already exists (409)");
    return;
  }
  if (!resp.ok) {
    const text = await resp.text();
...
async function pollOperation(operationName: string): Promise<void> {
  ...
  while (Date.now() - start < PROJECT_CREATION_TIMEOUT) {
    ...
    logger.info("Waiting for Lakebase project creation...", {
...
async function generateDbCredential(): Promise<string> {
  ...
  logger.debug("Lakebase DB credential generated", {
