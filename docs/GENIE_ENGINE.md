...
16. [Legacy Fallback](#legacy-fallback)
...
**Output:** `MetricViewProposal[]` (name, description, YAML, DDL, source
tables). These are shown in the Space Preview UI for review before manual
creation.
...
- **Name**: the metric view identifier
- **Description**: what the metric measures
- **YAML**: the metric view definition in Databricks format
- **DDL**: the SQL to create the metric view
- **Source tables**: which tables the metric view references

Metric view proposals are displayed in the Space Preview UI. They are
**not automatically deployed** -- they require manual review and creation
in Databricks.

Existing metric views discovered in the metadata are automatically included
in the space's `data_sources.metric_views` section.
...
| File | Purpose |
|---|---|
| `lib/dbx/genie.ts` | Genie Spaces REST API client |
